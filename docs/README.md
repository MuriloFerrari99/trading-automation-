...
2. **Opções na Alpaca:** a Wheel completa (vender CSP + covered call) exige apenas **Level 1**; contas paper normalmente vêm com nível alto liberado — **confirmar `acct.options_approved_level` no seu paper**. A options chain básica **não retorna delta**: é preciso calcular via Black-Scholes no cliente. (doc 04)
